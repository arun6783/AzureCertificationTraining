...
{
    public static class Http_ProductById
    {
        [FunctionName("ProductById")]
        public static async Task<IActionResult> Run(
            [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "products/{id:int}")] HttpRequest req, int id,
            ILogger log)
        {
         
                var products = new List<Product>();
                string productsStatement = $"select productID, ProductName, Quantity from Products where productId={id}";
